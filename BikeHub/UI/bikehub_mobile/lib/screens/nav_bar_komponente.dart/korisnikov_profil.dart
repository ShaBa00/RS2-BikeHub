    ...
    setState(() {
      futureKorisnik = _korisnikService.isLoggedIn().then((isLoggedIn) async {
        if (isLoggedIn) {
          ...
          grad.text = adresa?['grad'];
          postanskiBroj.text = adresa?['postanskiBroj'];
          ulica.text = adresa?['ulica'];

          username.text = korisnik?['username'];
          email.text = korisnik?['email'];
          imePrezime.text = korisnik?['korisnikInfos'][0]['imePrezime'] ?? '';
          telefon.text = korisnik?['korisnikInfos'][0]['telefon'] ?? '';
          return korisnik;
        }
        return null;
      });
