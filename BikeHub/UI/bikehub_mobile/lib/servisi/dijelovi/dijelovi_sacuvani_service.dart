...
class DijeloviSacuvaniServis {
  final logger = Logger();
  final KorisnikServis _korisnikService = KorisnikServis();
  final Dio _dio;

  DijeloviSacuvaniServis() : _dio = Dio() {
    configureDio(_dio);
  }
  ...
  Future<List<dynamic>?> getSacuvani({required int korisnikId}) async {
    ...
    Uri uri = Uri.parse('${HelperService.baseUrl}/SpaseniDijelovi');
  ...
  Future<Map<String, dynamic>?> isDioSacuvan(
      {required int korisnikId, required int dijeloviId}) async {
    await _addAuthorizationHeader();
    final Map<String, dynamic> queryParams = {
      'korisnikId': korisnikId.toString(),
  ...
  Future<String?> promjeniSacuvani(
      int sacuvaniId, int korisnikId, int dioId, bool obrisi) async {
    final String baseUrl =
        '${HelperService.baseUrl}/SpaseniDijelovi/$sacuvaniId';
    Uri uri;
  ...
  Future<String?> dodajNoviSacuvani(int dijeloviId, int korisnikId) async {
    if (dijeloviId == 0 && korisnikId == 0) {
      return 'Greška prilikom dodavanja zapisa';
    }
    final String baseUrl = '${HelperService.baseUrl}/SpaseniDijelovi';
    ...
    try {
      ...
      if (httpError is HttpException && httpError.message != null) {
        return 'Greška : ${httpError.message}';
      }
      return 'Greška  ${httpError.toString()}';
    } catch (e) {
      logger.e('Greška  $e');
      return e.toString();
    } finally {
